...
export function GNewsTodoPanel({ session }: { session: Session }) {
  ...
  return <section className="library-page">
    ...
    <form className="library-add" onSubmit={add}><label>New to-do<input value={title} onChange={(e) => setTitle(e.target.value)} minLength={2} maxLength={160} required placeholder="For example, prepare the evening announcement" /></label><button>Add to-do</button></form>
